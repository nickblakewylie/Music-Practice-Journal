  ...
  return (
    <Tab.Navigator
      screenOptions={({ route }) => ({
        ...
        tabBarBackground: () => <View style={style.absoluteFill} />,
        headerStyle: { backgroundColor: theme.colors.ACCENT },
      })}
    >
      <Tab.Screen
        name="Add Practice"
        options={{ headerTitle: () => <Header name="Add Practice" /> }}
      ...
      <Tab.Screen
        name="Goal"
        options={{ headerTitle: () => <Header name="Goals" /> }}
        component={GoalScreen}
      />
      <Tab.Screen
        name="Set List"
        options={{ headerTitle: () => <Header name="Set List" /> }}
...
export default function App() {
  ...
  const setListValue = React.useMemo(
    () => ({ setLists, setMySetLists }),
    [setLists, setMySetLists]
  );
  const pStateValue = React.useMemo(
    () => ({ updatePState, setUpdatePState }),
    [updatePState, setUpdatePState]
  );
  const providerValue = React.useMemo(
    () => ({ practiceSessions, setPracticeSessions }),
    [practiceSessions, setPracticeSessions]
  );
  const getData = async () => {
    ...
    getPracticeSession =
      getPracticeSession != null ? JSON.parse(getPracticeSession) : null;
