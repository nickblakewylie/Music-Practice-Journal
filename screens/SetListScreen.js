    ...
    function calculateThePracticeTimes(){
        ...
        if(setLists != null && practiceSessions != null){
            for(var i =0; i < newSetList.length; i ++){
                let newA = practiceSessions.filter(o => o.practiceSong == newSetList[i].songName)
                var time = 0;
                if(newA != null && newA.length > 0){
                    for(var p =0; p < newA.length; p ++){
                        if(newA[p].practiceTime != null && Number(newA[p].practiceTime) != null){
                    ...
                    <View style={[style.changeDifficultyModalStyle, {flexDirection:"column", justifyContent:"center"}]}> 
                        <View style={{flexDirection:"row", justifyContent:"center"}}>
                            ...
                            <View style={{flex: 0.4, alignItems:"flex-end"}}>
                                <TouchableOpacity onPress={() => setChangeDifficultyModal(false)} style={{justifyContent:"center", backgroundColor:theme.colors.ACCENT,padding: 10,borderTopRightRadius: 8}}>
                                    <Text style={{fontSize:theme.typography.size.M, color:theme.colors.TEXT}}>X</Text>
                                </TouchableOpacity>
                            </View>
                        </View>
                        <View style={{flexDirection:"column",alignSelf:"center", justifyContent:"center", width:"90%", marginTop: 20, marginBottom: 20}}>
                            <TextInput
                        ...
                        <View style={style.addSetListModalStyle}>
                            <View style={[style.setListModalContainer, {flexDirection: "column", justifyContent: "space-around"}]}>
                                <View style={{ flexDirection:"row", justifyContent:"center"}}>
                                    ...
                                    <View style={{flex: 0.4, alignItems:"flex-end"}}>
                                        <TouchableOpacity onPress={() => setAddSetListModal(false)} style={{justifyContent:"center", backgroundColor:theme.colors.ACCENT,padding: 10,borderTopRightRadius: 8}}>
                                            <Text style={{fontSize:theme.typography.size.M, color:theme.colors.TEXT}}>X</Text>
                                        </TouchableOpacity>
                                    </View>
                                </View>
                                <View style={{flexDirection:"column"}}>
                                        <View style={{flexDirection:"column", width: "90%", alignSelf:"center", margin: 15}}>
