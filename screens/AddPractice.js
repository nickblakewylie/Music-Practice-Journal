...
function AddPractice() {
  ...
  function checkInputsAreCorrect() {
    if (
      practiceTime != null &&
      Number.isInteger(Number(practiceTime)) &&
      Number(practiceTime) > 0 &&
      Number(practiceTime) < 1000
    ) {
      if (
        quality != null &&
        Number.isInteger(Number(quality)) &&
        Number(quality) > 0 &&
        Number(quality) < 11
      ) {
        return true;
  ...
  const downloadFile = async () => {
    if (checkInputsAreCorrect()) {
      ...
      if (videoUri != null) {
        ...
        const fileName =
          FileSystem.documentDirectory + "videos/" + uuid.v4() + ".mov";
  ...
  const storePracticeSession = async (newFileName) => {
    ...
    if (getPracticeSession == null) {
      ...
      await AsyncStorage.setItem(
        "practiceSessions",
        JSON.stringify(getPracticeSession)
      );
    } else {
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      ...
                      if (hasPermission !== true) {
                        Alert.alert(
                          "Oops",
                          "To record your practices you need to allow access to your camera",
                          [{ text: "OK" }]
                        );
                      } else {
            ...
            >
              {hasPermission ? (
                ...
                <View style={style.container}>
                  ...
                  >
                    <Text>Press for permission</Text>
                  </TouchableOpacity>
                </View>
              )}
            </Modal>
